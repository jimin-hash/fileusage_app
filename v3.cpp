				...
				if (!is_directory(d->status())) {
					...
					for (map<fileInfo, unsigned int>::iterator it = mFiles.begin(); it != mFiles.end(); it++) {		
						if (it->first.fileExp == files.fileExp) {
							cout << "it->first.countExp == " << it->first.countExp << "\t it->first.fileScale === " << it->first.fileScale << endl;
							cExp += it->first.countExp;
							capacity += it->first.fileScale;
							//capacity += file_size(d->path(), err);
							cout << cExp << " " << capacity << endl;

							(*it).first.fileScale = capacity;
							files.countExp = cExp;
					...
					mFiles.insert(pair<fileInfo, unsigned int>(files, cExp));
